...
caminho_dados <- "dados_junho/dados_processamento/empresas/"
print(caminho_dados)
...
for(i in 1:length(files_to_filter)){
  df <-  fread(file.path(caminho_dados, files_to_filter[i]), 
                    nrows=10000000, encoding="Latin-1") %>% tibble::as_tibble()
